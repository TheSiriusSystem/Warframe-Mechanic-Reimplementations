...
class TSS_HudMessageHandler : StaticEventHandler
{
	array < TSS_HudMessage > hudmessages;
	...
	override void NetworkProcess(consoleEvent e)
	{
		if (e.name == "testhudmsg")
		{
			...
			let hmsg = TSS_HudMessage.Create("$GOTSHOTGUN", id: 1, fontname: 'BigUpper', fontcolor: Font.CR_Green, alignment: TSS_HudMessage.ALIGN_CENTER, fadeinTime: 0, typeTime: 2, holdtime: 35, fadeOutTime: 50, scale: (0.5, 0.5), viewer: players[e.Player]);
			if (hmsg)
			{
	...
	override void RenderUnderlay(RenderEvent e)
	{
		for (int i = 0; i < hudmessages.Size(); i++)
		{
			...
			{
				//console.printf("Drawing message '%s' at %.1f, %.1f | alpha %.2f", hmsg.text, hmsg.pos.x, hmsg.pos.y, hmsg.alpha);
				Screen.DrawText(
...
class TSS_HudMessage play
{
	...
	static TSS_HudMessage Create(string text, uint id = 0, name fontname = 'NewSmallFont', int fontColor = Font.CR_Red, vector2 pos = (160, 50), int alignment = ALIGN_LEFT, uint fadeInTime = 0, uint typeTime = 0, uint holdTime = 35, uint fadeOutTime = 0, vector2 scale = (1,1), PlayerInfo viewer = null)
	{
		let hmsg = TSS_HudMessage(New("TSS_HudMessage"));
		if (hmsg)
		{
			hmsg.viewer = viewer;
			hmsg.scale = scale;
			...
			if (id > 0)
			{
				let handler = TSS_HudMessageHandler(StaticEventHandler.Find("TSS_HudMessageHandler"));
				if (handler)
				{
